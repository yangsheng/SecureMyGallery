...
@implementation SGPageViewController {
    ...
    UISwipeGestureRecognizer* swipeTodelete;
...
//    UIView *fakeNavView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 64)];
//    fakeNavView.backgroundColor = [UIColor lightGrayColor];
//    [self.view addSubview:fakeNavView];
    ...
    __strong SGPageBottomToolbar *bottomtoolbar = [[SGPageBottomToolbar alloc] initWithFrame:CGRectMake(0, s.height-90, s.width, kSGToolbarHeight)
                                                           browserDelegate:self];
    bottomtoolbar.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleBottomMargin;
    _bottomtoolbar.alpha=1.0;
    ...
    button.showsTouchWhenHighlighted = YES;
    //[button setTitle:NSLocalizedString(@"Remove Tab", @"") forState:UIControlStateNormal];
    [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [button setImage:[UIImage imageNamed:@"minus"] forState:UIControlStateNormal];
   button.titleEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 0);
    [button addTarget:self action:@selector(deleteTab:) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:button];
    _removeTabButton = button;
    ...
    __strong UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(0 , 0, self.view.bounds.size.width - 5, font.lineHeight)];
    ...
    swipeTodelete=[[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(handleSwipeGesture:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(siteIconAdded2:) name:@"SiteIconAdded" object:nil];
...
-(IBAction)siteIconAdded2:(id)sender{
...
//-(void)RemoveTab{
//
//    [NSThread detachNewThreadSelector:@selector(deleteTab:) toTarget:self withObject:nil];
//
//}
...
-(IBAction)deleteTab:(id)sender{
    ...
    while ( _viewControllers.count>1) {
        ...
        if (self.count==1) {
        //[self removeViewController:[_viewControllers objectAtIndex:i] withIndex:i];
            //_removeTabButton.enabled=NO;
            break;
        }
...
- (void)updateInterface {
    ...
    [_bottomtoolbar updateInterface];
...
- (void)_layout {
    ...
    if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_6_1) {
        ...
        _bottomOffset = [self.bottomLayoutGuide length];

    }
    CGFloat toolbarH = kSGToolbarHeight+_topOffset;
    CGFloat bottomtoolbarH = kSGToolbarHeight;

    if (_exposeMode) {
        _toolbar.frame = CGRectMake(0, -toolbarH, s.width, toolbarH);
        _bottomtoolbar.frame = CGRectMake(0, s.height-90, s.width, bottomtoolbarH);
    ...
    _removeTabButton.frame=CGRectMake(150, _topOffset + 10, 24 , 24);
...
//                CGFloat y1 = CGRectGetMaxY(bottomView.frame);
//                vc.view.frame = CGRectMake(size.width * i, y1,
//                                           size.width, size.height);
            } else {
...
- (void)_enableInteractions {
    for (NSUInteger i = 0; i < _viewControllers.count; i++) {
        ...
        [self.view removeGestureRecognizer:swipeTodelete];
...
- (void)_disableInteractions {
    for (NSUInteger i = 0; i < _viewControllers.count; i++) {
        ...
        [self.view addGestureRecognizer:swipeTodelete];
...
- (IBAction)_moveWebView:(UIPanGestureRecognizer *)recognizer {
    ...
    switch (recognizer.state) {
        ...
        case UIGestureRecognizerStateChanged:{
           ...
           // CGRect next2 = CGRectOffset(_bottomtoolbar.frame, 0, offset);

            if (next.origin.y >= -kSGToolbarHeight && next.origin.y <= 0) {
                _toolbar.frame = next;
                [_toolbar setSubviewsAlpha:1.f-fabs(next.origin.y/kSGToolbarHeight)];
                webC.view.frame = CGRectOffset(webC.view.frame, 0, offset);
//                webC.toolbar2.frame=next2;
//                
//                NSLog(@"%f",webC.toolbar2.frame.origin.y);
                int i=(int)trans.y;
                NSLog(@"%i",i);
//                _bottomtoolbar.frame = next2;

                [_bottomtoolbar setSubviewsAlpha:1.f-fabs(next.origin.y/kSGToolbarHeight)];
                ...
                if (i<-40) {
                    ...
                    _bottomtoolbar.alpha=1.0;
                    CGSize s = self.view.bounds.size;
                    _bottomtoolbar.frame=CGRectMake(0, s.height-90, s.width, kSGToolbarHeight);
                ...
                if ( curr.y > 0 && curr.y <= h) {
                    ...
                    _bottomtoolbar.frame=CGRectMake(0, s.height-50, s.width, kSGToolbarHeight);
                }
        ...
        case UIGestureRecognizerStateEnded:{
            ...
            if (y != -kSGToolbarHeight
                ...
                [_bottomtoolbar setSubviewsAlpha:1.0];
    ...
    if (touches == 1)
    {
        if (sender.state == UIGestureRecognizerStateEnded)
        {
            ...
            [UIView animateWithDuration:0.5 animations:^{
                webC.view.frame=CGRectOffset(webC.view.frame, 0, -500);
                
            } completion:^(BOOL finished) {
                [self removeViewController:webC withIndex:self.selectedIndex];
            }];
...
- (void)hideBottomToolBar{
    ...
    //SGBrowserViewController *browser = [[SGBrowserViewController alloc]init];

//    [browser setTabBarVisible:![browser tabBarIsVisible] animated:YES completion:^(BOOL finished) {
//            NSLog(@"finished");
//        }];
    
    //[browser hideTabBarController];
