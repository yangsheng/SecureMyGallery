...
- (void)performLoadUnderlyingImageAndNotify {
    ...
    if (_image) {
        ...
        NSString *_strScheme = [_photoURL scheme];
        ...
        } else if ([_strScheme isEqualToString:@"file"]) {
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                @autoreleasepool {
                    @try {
                        ...
                        {
                            self.underlyingImage = [UIImage imageWithContentsOfFile:[self.dataSource getThumbnail480PathForFilename:[_photoURL.path lastPathComponent]]];
                        }
                        else
