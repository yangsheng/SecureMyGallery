...
- (void)viewDidLoad {
    ...
    if (self.displayNavArrows) {
        ...
        _deleteButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"mwDelete"] style:UIBarButtonItemStylePlain target:self action:@selector(deleteButtonPressed:)];
    }
    if (self.displayActionButton) {
        _actionButton = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"mwShare"] style:UIBarButtonItemStylePlain target:self action:@selector(actionButtonPressed:)];
    }
    
    UIImage *mwBackArrowD = [[UIImage imageNamed:@"mwBackArrowD"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    [_previousButton setImage:mwBackArrowD];
    
    UIImage *mwForwardArrow = [[UIImage imageNamed:@"mwForwardArrow"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    [_nextButton setImage:mwForwardArrow];
    
    UIImage *mwDelete = [[UIImage imageNamed:@"mwDelete"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    [_deleteButton setImage:mwDelete];
    
    UIImage *mwShare = [[UIImage imageNamed:@"mwShare"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
    [_actionButton setImage:mwShare];
...
- (void)setNavBarAppearance:(BOOL)animated {
    ...
    navBar.tintColor = SYSTEM_VERSION_GREATER_THAN_OR_EQUAL_TO(@"7") ? NAV_TINT_COLOR : nil;
    if ([navBar respondsToSelector:@selector(setBarTintColor:)]) {
        navBar.barTintColor = NAV_BAR_TINT_COLOR;
...
- (void)updateNavigation {
    ...
    if (_previousButton.enabled) {
        UIImage *mwBackArrow = [[UIImage imageNamed:@"mwBackArrow"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        [_previousButton setImage:mwBackArrow];
    }else{
    ...
    if (_nextButton.enabled) {
        ...
        UIImage *mwForwardArrowD = [[UIImage imageNamed:@"mwForwardArrowD"] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal];
        [_nextButton setImage:mwForwardArrowD];
    }
    ...
    _deleteButton.enabled = _actionButton.enabled;
...
////        {
////            [[AdManager sharedInstance] animate_banner:990];
////
////        }
////        else
////        [[AdManager sharedInstance] animate_banner:545];
//        
//    }
//    else
//    {
    ...
    if (_deleteSheet) {
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            self.deleteSheet = [[UIActionSheet alloc] initWithTitle:nil
                                              ...
                                              destructiveButtonTitle: @"Delete Photo" otherButtonTitles: nil];
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([self.delegate respondsToSelector:@selector(photoBrowser:actionButtonPressedForPhotoAtIndex:)]) {
                ...
                if (SYSTEM_VERSION_LESS_THAN(@"6")) {
                    ...
                    MWPhoto *_photoShare = photo;
                    //NSData *_data = [NSData dataWithContentsOfFile:_photoShare.photoURL.path];//kch-share-video
                    NSMutableArray *items = [NSMutableArray arrayWithObject:_photoShare.photoURL];
                    if (photo.caption) {
...
- (void)deletePhoto {
    ...
    if ([photo underlyingImage]) {
        //Delete photo
        if ([[NSUserDefaults standardUserDefaults]boolForKey:@"deleteDImage"]) {
            PGDataSource *ds = [[PGDataSource alloc] initWithCaller];
            [ds deleteFileWithName: photo.fileName Folder: photo.folderName];

        }else{
            PGDataSource *ds = [[PGDataSource alloc] init];
            [ds deleteFileWithName: photo.fileName Folder: photo.folderName];
        ...
        //CurrentPageIndex
        if (_currentPageIndex == [self numberOfPhotos])
            _currentPageIndex--;
