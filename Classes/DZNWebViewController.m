...
#define iPhone_navBar_Padding 44.0f
#define iPhone_Padding 44.0f
#define iPad_Padding 100.0f

static char DZNWebViewControllerKVOContext = 0;

@interface DZNWebViewController ()<UITextFieldDelegate>{
...
@property (nonatomic, strong) UIBarButtonItem *dismissBarItem;
@property (nonatomic, strong) UIBarButtonItem *cancelBarItem;
@property (nonatomic, strong) UIBarButtonItem *browserBarItem;
...
- (void)commonInit
{
    ...
    self.webView = [[DZNWebView alloc] initWithFrame:testview.bounds configuration:[WKWebViewConfiguration new]];
...
- (void)loadView
{
    ...
    self.view.frame=testview.bounds;
...
- (void)viewDidLoad
{
    ...
    [txtField setFrame:CGRectMake(30, 0, self.navigationController.navigationBar.frame.size.width, 32)];
    [txtField setBorderStyle:UITextBorderStyleNone];
    txtField.text=[self.URL absoluteString];
    txtField.delegate =self;
    txtField.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
    txtField.textColor = [UIColor whiteColor];
    txtField.textAlignment = NSTextAlignmentLeft;
    txtField.borderStyle = UITextBorderStyleRoundedRect;
    txtField.font = [UIFont fontWithName:@"Helvetica" size:15];
    txtField.autocorrectionType = UITextAutocorrectionTypeNo;
    txtField.contentVerticalAlignment = UIControlContentVerticalAlignmentCenter;
    txtField.clearsOnBeginEditing = NO;
    txtField.autocapitalizationType = UITextAutocapitalizationTypeNone;
    txtField.autocorrectionType = UITextAutocorrectionTypeNo;
    txtField.keyboardType = UIKeyboardTypeWebSearch;
    txtField.returnKeyType = UIReturnKeySearch;
    [txtField setBackgroundColor:[UIColor colorWithPatternImage:[UIImage imageNamed:@"search_bg"]]];
    txtField.clearButtonMode = UITextFieldViewModeWhileEditing ;
    [txtField setTintColor:[UIColor whiteColor]];
    [txtField setTintAdjustmentMode:UIViewTintAdjustmentModeNormal];

    

    // reloadButton
    stopReloadButton = [UIButton buttonWithType:UIButtonTypeCustom];
    stopReloadButton.frame = CGRectMake(0, 0, 30, 20);
    [stopReloadButton setImage:[UIImage imageNamed:@"reload"] forState:UIControlStateNormal];
    [stopReloadButton setImage:[UIImage imageNamed:@"reload"] forState:UIControlStateHighlighted];
    ...
    txtField.rightView = stopReloadButton;
    txtField.rightViewMode = UITextFieldViewModeUnlessEditing;

    //Navigation
    
    //[Flurry logEvent:@"webview_page"];


  
    NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"hide" forKey:@"pos_Key"];
...
dispatch_async(dispatch_get_main_queue(), ^{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        [self.navigationItem setRightBarButtonItem:[self cancelBarItem] animated:YES];
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField {
    ...
    if (DZN_IS_IPAD) {
        ...
        [UIView animateWithDuration:0.3 animations:^{
            ...
            [UIView animateWithDuration:0.3 animations:^{
                [_cancelBarItem setTintColor:[UIColor lightGrayColor]];
...
-(void) cancelbtnTapped:(id)sender{
    ...
    [UIView animateWithDuration:0.3 animations:^{
        [txtField resignFirstResponder];
        [txtField setFrame:CGRectMake(0, 0, self.navigationController.navigationBar.frame.size.width, 32)];
        self.navigationItem.rightBarButtonItem = Nil;
        [_cancelBarItem setTintColor:[UIColor clearColor]];
        _cancelBarItem.enabled = NO;

        [self.navigationItem setLeftBarButtonItem:[self dismissBarItem] animated:YES];
    }];
    }
}
...
- (void)finishSearch:(NSString *)searchString title:(NSString *)title {
    ...
    if ([searchString hasPrefix:@"http"] || [searchString hasPrefix:@"www"]) {

    [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@",searchString]]]];
    }
    else{
        [self.webView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"https://www.google.com/search?q=%@",[searchString stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]]]]]];
...
- (UIBarButtonItem *)dismissBarItem
{
    if (!_dismissBarItem)
    {
        _dismissBarItem = [[UIBarButtonItem alloc] initWithImage:[self dismissButtonImage] landscapeImagePhone:nil style:UIBarButtonItemStylePlain target:self action:@selector(dismissVc:)];
        _dismissBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Dismiss", @"DZNWebViewController", @"Accessibility label button title");
        _dismissBarItem.enabled = YES;
    }
    return _dismissBarItem;
}
...
- (UIBarButtonItem *)cancelBarItem
{
    if (!_cancelBarItem)
    {
        _cancelBarItem = [[UIBarButtonItem alloc] initWithTitle:@"Cancel" style:UIBarButtonItemStylePlain target:self action:@selector(cancelbtnTapped:)];
        _cancelBarItem.accessibilityLabel = NSLocalizedStringFromTable(@"Cancel", @"DZNWebViewController", @"Accessibility label button title");
        _cancelBarItem.enabled = NO;
        [_cancelBarItem setTintColor: [UIColor clearColor]];
...
- (UIBarButtonItem *)stateBarItem
{
    if (!_stateBarItem)
    {
        _stateBarItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"home"] landscapeImagePhone:nil style:0 target:self  action:@selector(goHome:)];
//        [self updateStateBarItem];
        _stateBarItem.enabled = YES;
    }
    return _stateBarItem;
}

- (UIBarButtonItem *)browserBarItem
{
    if (!_browserBarItem)
    {
        _browserBarItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"tab"] landscapeImagePhone:nil style:0 target:self  action:@selector(goBrowser:)];
        //        [self updateStateBarItem];
        _browserBarItem.enabled = YES;
    }
    return _browserBarItem;
}
...
- (NSArray *)navigationToolItems
{
    ...
    if (self.supportedWebActions > 0) {
        if (!DZN_IS_IPAD) [items addObject:flexibleSpace];
        [items addObject:self.browserBarItem];
    }
...
#pragma mark Bar Button Image

- (UIImage *)dismissButtonImage
{
    if (!_dismissButtonImage) {
        _dismissButtonImage = [UIImage imageNamed:@"dismiss"];
    }
    return _dismissButtonImage;
}

- (UIImage *)backwardButtonImage
{
    if (!_backwardButtonImage) {
        _backwardButtonImage = [UIImage imageNamed:@"back_color" inBundle:nil compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)forwardButtonImage
{
    if (!_forwardButtonImage) {
        _forwardButtonImage = [UIImage imageNamed:@"forward_color" inBundle:nil compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)reloadButtonImage
{
    if (!_reloadButtonImage) {
        _reloadButtonImage = [UIImage imageNamed:@"reload" inBundle:nil compatibleWithTraitCollection:nil];
    }
...
{
    if (!_stopButtonImage) {
        _stopButtonImage = [UIImage imageNamed:@"clear" inBundle:nil compatibleWithTraitCollection:nil];
    }
...
- (UIImage *)actionButtonImage
{
    if (!_actionButtonImage) {
        _actionButtonImage = [UIImage imageNamed:@"linkshare" inBundle:nil compatibleWithTraitCollection:nil];
    }
...
-(void) goBrowser: (id)sender{
    UIAlertController* alert = [UIAlertController alertControllerWithTitle:nil
                                                                   message:nil
                                                            preferredStyle:UIAlertControllerStyleActionSheet];
    UIAlertAction* chromeAction = [UIAlertAction actionWithTitle:@"Open in Chrome" style:UIAlertActionStyleDefault
                                                       handler:^(UIAlertAction * action) {
                                                           
                                                           [self openInChrome:nil];
                                                           
                                                       }];
    
    UIAlertAction* safariAction = [UIAlertAction actionWithTitle:@"Open in Safari" style:UIAlertActionStyleDefault
                                                        handler:^(UIAlertAction * action) {
    ...
    [alert setShouldGroupAccessibilityChildren:YES];
...
- (void)performActivity {
    NSString *openingURL = encodeByAddingPercentEscapes(self.webView.URL.absoluteString);
    NSString *callbackURL = encodeByAddingPercentEscapes([[NSURL URLWithString:@"qrbot://"] absoluteString]);
    NSString *sourceName = encodeByAddingPercentEscapes([[NSBundle mainBundle]objectForInfoDictionaryKey:@"CFBundleName"]);
    NSURL *activityURL = [NSURL URLWithString:[NSString stringWithFormat:@"googlechrome-x-callback://x-callback-url/open/?url=%@&x-success=%@&x-source=%@&create-new-tab", openingURL, callbackURL, sourceName]];
    NSLog(@"%@",activityURL);
...
-(void) dismissVc: (id)sender{
 ...
 //   NSDictionary *userInfo = [NSDictionary dictionaryWithObject:@"top_of_tab" forKey:@"pos_Key"];
    [[NSNotificationCenter defaultCenter] postNotificationName: @"full_screenCall" object:nil userInfo:nil];
...
- (void)configureToolBars
{
    ...
    [self.navigationController.toolbar setBarTintColor:[UIColor colorWithRed:(41/256.0) green:(41/256.0) blue:(41/256.0) alpha:(1.0)]];
    ...
    self.navigationItem.leftBarButtonItem = [self dismissBarItem];
...
- (void)webView:(DZNWebView *)webView didFinishNavigation:(WKNavigation *)navigation
{
    ...
    txtField.text=[self.webView.URL absoluteString];
    [self updateLoadingStatus];
    
//    static dispatch_once_t webConfig;
//
//    dispatch_once(&webConfig, ^{
//        [UIView animateWithDuration:0.3 animations:^{
...
//                fr.size = CGSizeMake(self.view.frame.size.width, screenHeight-64-iPhone_Padding-iPhone_navBar_Padding);
//                
//            }
//            else{
//                fr.size = CGSizeMake(self.view.frame.size.width, screenHeight-64-iPad_Padding);
//            }
//            webView.frame = fr;
//        }];
//    });
